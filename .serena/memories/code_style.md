...
- Documentation: Sphinx/Read the Docs sources now live under `docs_rtd/` (MyST, Furo theme). Continue to keep the legacy mdBook (`docs/src`) consistent when touching overlapping pages, but prioritize updating the Sphinx tree and re-running `sphinx-build -b html docs_rtd docs_rtd/_build/html` when behavior or APIs change.
