...
// Well, I don't know how to do this without copy
[[nodiscard]] Score<Second> parse_midi_sec(const minimidi::file::MidiFile &midi) {
    typedef Second::unit unit;
    // remove this redundant copy in the future
    const u16 tpq = midi.get_tick_per_quarter();
    Score<Second> score(tpq); // create a score with the given ticks per quarter
    namespace message = minimidi::message; // alias

    // pre load all the tempo changes
    vec<Tempo<Tick>> tick_tempos;
    for(const auto &midi_track: midi.tracks) {
        for(const auto &msg: midi_track.messages) {
            if (msg.get_type() == message::MessageType::Meta
                && msg.get_meta_type() == message::MetaType::SetTempo) {
                tick_tempos.emplace_back(msg.get_time(), msg.get_tempo());
            }
        }
    }
    ops::sort_by_time(tick_tempos);
    // add guard at begin and end
    if(tick_tempos.empty() || tick_tempos.front().time != 0)
        tick_tempos.emplace(tick_tempos.begin(), 0, 500000);
    tick_tempos.emplace_back(std::numeric_limits<Tick::unit>::max(), tick_tempos.back().mspq);

    // init other helpers
    // (channel, pitch) -> (start, velocity)
    NoteOnQueue<Second> last_note_on[16][128] = {};

    for(const auto &midi_track: midi.tracks) {
        // (channel, program) -> Track
        std::map<TrackIdx, Track<Second>> track_map;
        // channel -> Track
        Track<Second> stragglers[16];
        Track<Second>* last_track = nullptr;
        u8 last_channel = 255, last_program = 255;
        // channel -> program
        uint8_t cur_instr[16] = {0};
        std::string cur_name;
        ...
        // build a function to convert tick to unit
        i32 pivot_tick = 0;
        f32 pivot_time = 0;
        auto t_iter = tick_tempos.begin() + 1;
        f64 cur_factor = static_cast<f64>(tick_tempos[0].mspq) / 1000000. / static_cast<double>(tpq);

        for(const auto &msg: midi_track.messages) {
            const auto cur_tick = static_cast<Tick::unit>(msg.get_time());
            // const auto cur_time = tick2unit(cur_tick);
            if(cur_tick > t_iter->time) {
            ...
            const auto cur_time = pivot_time + static_cast<float>(cur_factor * (cur_tick - pivot_tick));
            switch (msg.get_type()) {
                case (message::MessageType::NoteOn): {
                    if (uint8_t velocity = msg.get_velocity(); velocity != 0) {
                        auto pitch = msg.get_pitch();
                        ...
                        if (velocity >= 128) throw std::range_error(
                            "Get velocity=" + std::to_string(static_cast<int>(velocity)));
                        last_note_on[msg.get_channel()][pitch].emplace(
                            cur_time, static_cast<i8>(velocity));
                        break;
                    } // if velocity is zero, turn to note off case
                }
                case (message::MessageType::NoteOff): {
                    ...
                    if ((!note_on_queue.empty()) && (cur_time > note_on_queue.front().time)) {
                        auto const &note_on = note_on_queue.front();
                        track->notes.emplace_back(
                            note_on.time, cur_time - note_on.time,
                            static_cast<i8>(pitch), note_on.velocity
                        );
                        note_on_queue.pop();
                    }
